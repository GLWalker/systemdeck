 ...
 * @since 1.1.0
 * @author G.L. Walker
 * @file wp-content/plugins/systemdeck/assets/js/sd-player-app.js
...
;(function ($) {
	...
	const ensureModalPlayerAssets = () => {
		const playerScript = document.querySelector(
			'script[src*="assets/js/sd-player-app.js"]',
		)
		const playerScriptSrc = String(playerScript?.src || "")
		if (playerScriptSrc.includes("/assets/js/sd-player-app.js")) {
			const baseUrl = playerScriptSrc.split("/assets/js/sd-player-app.js")[0]
			if (baseUrl) {
				ensureStylesheet(
					`${baseUrl}/assets/css/systemdeck-player.css`,
	...
	const sdPrimeAudioContextFromGesture = () => {
	...
	const PlayerUI = {
		...
		bindMiniPlayerSurface(surfaceRoot) {
			...
			ui.play?.addEventListener("click", async () => {
				sdPrimeAudioContextFromGesture()
				const state = window.SystemDeckPlayback?.getState?.()
		...
		updateUI(ui, state) {
			...
			const explicitBusy =
				...
				this.runtimeState?.isLoading === true ||
				this.runtimeState?.isHydrating === true ||
				status === "loading"
			const controlsDisabled = !runtimeTrackUrl || explicitBusy
			...
			if (ui.play) ui.play.disabled = controlsDisabled
			if (ui.pause) ui.pause.disabled = controlsDisabled
			if (ui.prev) ui.prev.disabled = controlsDisabled
			if (ui.next) ui.next.disabled = controlsDisabled
			...
			if (autoplayBtn) autoplayBtn.disabled = controlsDisabled
			if (repeatTrackBtn) repeatTrackBtn.disabled = controlsDisabled
			...
			if (ui.playlist && item.source) {
				...
				if (item.source && miniSurface) {
					...
					if (window.SystemDeckAudio?.getEQState) {
						this.syncAllEQUI(window.SystemDeckAudio.getEQState())
					}
					setTimeout(() => {
						const modalTrackUrl = String(
							this.runtimeState?.currentTrack?.url ||
								this.runtimeState?.currentTrack?.source ||
								"",
						)
						if (!modalTrackUrl) return
						this.updateRuntimeState({
							isBusy: false,
							isLoading: false,
							isHydrating: false,
						...
						this.updateUI(ui, this.buildModalSeededPlaybackState(item))
					}, 50)
					if (window.SystemDeckAudioDebug === true) {
						console.debug("[SystemDeckPlayer:modal]", {
						stage: "mounted",
		...
		buildPresetEQState(preset = {}, baseState = {}) {
			...
			const title = String(
				...
				const groupLabel = String(group || "").trim().startsWith("Workspace:")
					? String(group || "").trim()
					:
					origin === "builtin"
						? "Built-in"
			...
			try {
				...
				if (response.success) {
					...
					if (Array.isArray(data.items) && data.items.length > 0) {
						...
						const workspacePinned = this.getWorkspacePinnedPlaylist(ui)
						if (workspacePinned.length) {
							playlist = [...playlist, ...workspacePinned]
						}
		...
		init() {
			...
			this.registerCoreModalListeners()
			this.initialized = true
			this.drainPendingModalMounts()
		...
		registerCoreModalListeners() {
			if (this.coreListenersRegistered) return
			document.addEventListener("systemdeck:player-modal-mount", (e) => {
				const detail = e.detail || {}
				if (window.SystemDeckAudioDebug === true) {
					console.debug(
						"[SystemDeckPlayer] modal mount",
						detail.context || "modal",
						detail.trackId || "",
					)
				}
				if (!this.initialized) {
					this.pendingModalMounts.push(detail)
					window.__sdPendingPlayerMounts = this.pendingModalMounts
					return
				}
				this.mountModalSurface(detail)
			})
			...
			if (window.SystemDeckAudioDebug === true) {
				console.debug("[SystemDeckPlayer] core listeners registered")
			}
		},

		drainPendingModalMounts() {
			const queue = Array.isArray(this.pendingModalMounts)
				? this.pendingModalMounts.slice()
				: []
			this.pendingModalMounts = []
			window.__sdPendingPlayerMounts = this.pendingModalMounts
			queue.forEach((detail) => this.mountModalSurface(detail || {}))
		},
	}
